...
build:
	docker build \
	    ...
	    --build-arg VCS_REF=`git rev-parse --short HEAD` \
            --build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` \
	    -t $(DOCKER_REPO)/$(DOCKER) .
#	    -t $(DOCKER_REPO)/$(DOCKER):$(VERSION) .

push: build
	docker push $(DOCKER_REPO)/$(DOCKER)
	
pull:
	docker pull $(DOCKER_REPO)/$(DOCKER)
...
run:
	docker run --name=$(NAME) --restart=$(RESTART_OPTION) $(VOLUME_MAP) $(DOCKER_REPO)/$(DOCKER)
